...
def create_app():
    ...
    def register():
        ...
        user = User(name=data.get('name'), email=data['email'], password_hash=pw_hash, role=data.get('role','developer'))
        db.session.add(user)
        db.session.commit()
    ...
    def login():
        ...
        return jsonify({'access_token': access, 'user': {'id':user.id, 'name':user.name, 'email':user.email, 'role':user.role}})
    ...
    def update_bug(bid):
        ...
        if 'title' in data: bug.title = data['title']
        if 'description' in data: bug.description = data['description']
        if 'status' in data: bug.status = data['status']
        if 'priority' in data: bug.priority = data['priority']
        if 'assignee_id' in data: bug.assignee_id = data['assignee_id']
        db.session.commit()
